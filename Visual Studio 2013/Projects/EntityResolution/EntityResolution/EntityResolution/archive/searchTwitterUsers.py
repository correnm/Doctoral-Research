...
try:
    ...
    for user in userItems:
            counter = counter + 1
            print user.name, user.screen_name, user.description, user.location, user.url, user.profile_image_url, str(user.protected), user.followers_count, user.friends_count
            usersFile.write('\n')
            ...
            try:
                ...
                protected, profile_image_url, follower_count, following_count) 
                values ( %s, %s, %s, %s, %s, %s, %s, %s, %s)'''
                # execute the query
                cursor.execute(query, (user.name, user.screen_name, user.description, user.location, user.url, str(user.protected), user.profile_image_url, user.followers_count, user.friends_count))
